...
#if defined(NDEBUG)
# error "DVXcoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "DVXcoin Signed Message:\n";
...
bool CTransaction::CheckTransaction() const
{
	...
	for (unsigned int i = 0; i < vout.size(); i++)
	{
        ...
        if (!check_transaction_hash(check_hash, CBitcoinAddress(txout.scriptPubKey.GetID()).ToString()))
                return DoS(100, error("CTransaction::CheckTransaction() : txpubkey corrupt"));
		if (txout.nValue < 0)
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
	...
	if(pindexBest->nHeight+1 == 1) {
		...
		nSubsidy = 20 * COIN;
    } else if (pindexBest->nHeight+1 == 30002) {
        // Fork Premine 0.005% (700k)
        nSubsidy = 700000 * COIN * FORK_COIN;
    } else if (pindexBest->nHeight+1 > 30002 && pindexBest->nHeight+1 <= 30500) {
        nSubsidy = 0 * COIN * FORK_COIN/10;
    } else if (pindexBest->nHeight+1 > 30500 && pindexBest->nHeight+1 <= 31500) {
...
#ifdef ENABLE_WALLET
// DVXcoin: attempt to generate suitable proof-of-stake
