...
static const int64_t MAX_MONEY = 1500000000 * COIN; // 1,5 Billions CAP
static const int64_t FORK_RATE = 1000;
...
inline int64_t GetCoinYearReward(int nHeight) {
    LogPrint("getcoinrewards", "Block: nHeight=%u\n", nHeight);
    ...
    if (!TestNet() && nHeight < 1000)
        ...
        YearPercent = 10000;
    else if (nHeight <= 37000)
        YearPercent = 1;
    else if (nHeight >= 38000 && nHeight <= 50000)
        YearPercent = 1;
    else
        if (lastDigits == 25 || lastDigits == 50 || lastDigits == 75)
            YearPercent = 500;
