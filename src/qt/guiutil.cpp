...
{
    // Convert DVXcoin:// to DVXcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("DVXcoin://"))
    {
        uri.replace(0, 12, "DVXcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "DVXcoin.lnk";
}
...
{
    return GetAutostartDir() / "DVXcoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("DVXcoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  DVXcoin-qt [" + tr("command-line options") + "]                     " + "\n";
