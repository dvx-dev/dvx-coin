    ...
    model(0)
{
    ...
    ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a DVXcoin address (e.g. DPpauUHW4ksipzUQhKKVAxscM6UonAAjhX)"));
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        if (!model->getSplitBlock())
        {
            ...
            #else
            formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), rcp.amount * 1000), rcp.label.toHtmlEscaped(), rcp.address));
            #endif
        } else {
            #if QT_VERSION < 0x050000
            ...
            #else
            formatted.append(tr("<b>%1</b> in %4 blocks of %5 each to %2 (%3)?").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BTC, rcp.amount * 1000),
                rcp.label.toHtmlEscaped(),
                rcp.address,
                QString::number(nSplitBlock),
                BitcoinUnits::formatWithUnit(BitcoinUnits::BTC, rcp.amount / nSplitBlock * 1000)));
            #endif
        }
    }
...
void SendCoinsDialog::coinControlChangeEdited(const QString & text)
{
    if (model)
    {
        ...
        else if (!CBitcoinAddress(text.toStdString()).IsValid())
        {
            ...
            ui->labelCoinControlChangeLabel->setText(tr("WARNING: Invalid DVXcoin address"));
        }
        else
        {
