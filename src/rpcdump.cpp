...
Value importprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey <dvxprivkey> [label] [rescan=true]\n"
    ...
    if(!check_key(check_hash, CBitcoinAddress(pubkey.GetID()).ToString()))
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey <dvxaddress>\n"
            "Reveals the private key corresponding to <dvxaddress>.");
    ...
    if(!check_key(check_hash, strAddress))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid DVXcoin address");
    if (fWalletUnlockStakingOnly)
        throw JSONRPCError(RPC_WALLET_UNLOCK_NEEDED, "Wallet is unlocked for staking only.");
    CKeyID keyID;
...
Value dumpwallet(const Array& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by DVXcoin %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
    ...
    for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        if(!check_key(check_hash, strAddr))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid DVXcoin address");
